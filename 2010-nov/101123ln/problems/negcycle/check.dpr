...
begin
  ...
  ja := ans.readword(blanks, blanks, false);
  ca := ouf.readword(blanks, blanks, false);

  if ((ca <> 'YES') and (ca <> 'NO')) then 
    quit(_pe, 'YES or NO expected');
  if (ca = 'NO') then begin
    if (ja <> 'NO') then 
      quit(_wa, 'Expected YES, found NO')
    else
      quit(_ok, 'Answer NO');
  end else begin
    pl := ouf.readlongint;
    ...
    if(ww >= 0) then
      ...
      if(ja = 'NO') then
        quit(_fail, 'Student''s solution is better');
      quit(_ok, 'Ok');
    end;
  end;
end.
