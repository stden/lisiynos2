...
procedure InitFromStr( Var L:Long; N:String );
var i : integer;
begin
  fillChar(L,sizeOf(L),0);
  { Знак }
  if N='0' then L[-1]:=0
  else if N[1]<>'-' then L[-1]:=1
  else L[-1] := -1;
  for i := Length(N) downto 1 do
    L[Length(N)-i] := StrToInt(N[i]);
end;
...
(*  for i:=1 to 22 do
  ...
  assign(input,'j1.in'); reset(input);
  assign(output,'j1.out'); rewrite(output);
  readln(A);
  readln(B);
  InitFromStr(AA,A);
  InitFromStr(BB,B);
